...
**我们是公司的感官系统。** 模型团队是大脑，我们是眼睛、耳朵和神经网络。

```
现场传感器  →  DataFactory Pipeline  →  干净数据  →  模型团队  →  产品决策
   眼睛          视网膜 + 视神经         结构化信号       大脑          行动
```

人脑擅长多模态处理——触觉、视觉、听觉——但前提是**正确的数据**持续稳定地流入。我们的工作是同一件事：把高质量数据（质检通过、去重、过滤）持续喂给"大脑"，驱动模型迭代。算法选型是模型团队的职责；我们的职责是理解他们的需求（"更多夜间场景"、"polygon 标注而非 bbox"），然后调整 pipeline 去满足。
...
**全链路职责**：不只是写代码——还包括到现场部署硬件、维护传感器、评估新型摄像头/LiDAR/振动传感器、跟运营团队对接部署需求。软件 + 硬件 + 现场 = 真正理解 AI 落地的全貌。
...
**MLOps Engineer 的完整职责范围：**

| 维度 | 具体内容 |
|------|----------|
| 数据工程 | Pipeline 开发、质量保障、去重、标注闭环 |
| 现场运维 | 传感器安装/调试、网络联调、边缘服务器部署 |
| 传感器选型 | 评估新摄像头（分辨率/帧率/夜视/防尘级别）、LiDAR、振动传感器 |
| 边缘部署 | 把 pipeline 容器化部署到矿山现场，确保离线可运行 |
| 跨团队协作 | 对接模型团队数据格式需求、对接运营团队部署计划 |
...
## ⚡ 高效筛选（Smart Frame Selection Pipeline）

**设计理念**：五层串联过滤器，每层独立开关，逐级压缩数据量。

```
全部帧 → I 帧过滤 → 运动唤醒 → Best-frame → 场景变化去重 → 跟踪去重
  1800       ~15         ~10        top-K        unique scenes    1 per target
```

| # | 技术 | 作用 | 依赖 | 状态 |
|---|------|------|------|------|
| 1 | **I-frame 提取** | 只解关键帧，跳过 P/B 帧（画质最高 + seek 最快） | ffprobe + OpenCV | ✅ 完成 |
| 2 | **运动唤醒** | 静止画面跳过 YOLO（帧差/光流低于阈值） | OpenCV diff/flow | ✅ 完成 |
| 3 | **Best-frame selection** | score = mean(conf) × sharpness，每视频只保留 top-K 帧 | YOLO + quality_tools | ✅ 完成 |
| 4 | **级联检测** | 轻量模型先筛，有目标才跑大模型 | 轻量 .pt / ONNX | ✅ 完成 |
| 5 | **场景变化检测** | 相邻帧 CLIP embedding / 直方图差异突变时才取帧，防同场景重复 | CLIP（已集成） | ⬜ v4 TODO |
| 6 | **目标跟踪去重** | ByteTrack/BoT-SORT 跟踪，每个 track ID 只保留最优帧（"一辆车一张图"） | ultralytics model.track() | ⬜ v4 TODO |
| 7 | **Embedding/Re-ID** | 向量检索"谁在哪里"，跨视频去重与目标关联 | 向量 DB、Re-ID 模型 | ⬜ v4 TODO |

**当前水平**：1-4 已实现（中级），terminal 自动显示采样策略组合。
**v4 目标**：加入 5-6 后达到高级水平——场景变化检测防冗余，目标跟踪实现"一目标一帧"，数据量再压缩 5-10×。
