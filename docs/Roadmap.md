...
│  (1b) 损坏视频 [v2/v3]  可读性检测 → 移入 corrupt 或删除                          │
│  (2) 不合格检测 [已实现] blur/brightness/jitter + 可扩展(register_extra_check)     │
└─────────────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  复核 (Review) [已实现]  邮件汇总 → Terminal y/n 或 厂长中控台 Web 复核（入队无超时）  │
└─────────────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
...
**统一架构思维**：流程（Ingest → QC → Review → Archive）+ 工具类（engines/）+ 决策在 core/ + 配置（config/）。工具只返回数值，决策层读配置做判断。
...
我们是造**整车**的，模型团队是造**发动机**的。我们负责产线（Ingest → QC → Review → Archive）、配置、部署与可观测性；他们负责模型训练与发布。我们**留接口**（如 `run_vision_scan(cfg, video_paths)`），模型方提供 `.pt` 或实现接口即可接入。部署到 Edge 时仍是同一套 pipeline + 同一份配置，责任清晰。
...
*文档版本：v2026.02 | 版本线：v1 → v1.5 → v2 → v2.5 → v2.6 → v2.7 → v3 → v4 | 与工业/矿业 AI 安全、效率与数据质量需求对齐*
