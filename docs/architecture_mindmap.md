...
| **Ingest 预检** | dedup + 首帧解码；失败项移入 `quarantine/duplicate`、`quarantine/decode_failed` |
| **SSOT** | 批次目录名、前缀、后缀均在 settings.yaml |
| **Validation Layer** | `validate_config()`：min&lt;max、gate 范围、双门槛一致性 |
| **Storage Hierarchy** | raw → archive/rejected/redundant；quarantine（预检）；Batch 内 reports/source/refinery/inspection |
...
| **Backoff Retry** | `retry_utils.safe_move_with_retry`；`attempt × backoff_seconds` |
...
| **Batch Metrics** | `GET /api/metrics`；`engines/metrics.py` |
| **Labeling Export** | `scripts/export_for_labeling.py`；`storage/for_labeling` |
| **Consistency Validation** | `import_labeled_return.py`；IoU 匹配；一致率门槛 |
| **Version Mapping** | `version_info.json`；algorithm_version / vision_model_version |

---

*文档版本：v2.7 | 与 README Design philosophy 对应*
